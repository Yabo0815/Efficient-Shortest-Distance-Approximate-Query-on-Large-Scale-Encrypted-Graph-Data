...
int compute_BFS_for_District(map<int, vector<int> >& _graph, map<int,int>& OneMap,int k,int center) {
  ...
  OneMap[center_nodeid] = 0;
  cout << "set map[" << center_nodeid << "] as " << OneMap[nodeid] << endl;
  ...
  while(!que_tmp_node.empty()) {
      ...
      for(int i=0;i<vecNode.size();i++) {
          nodeid = vecNode[i];
          if(!OneMap[nodeid]) {
            OneMap[nodeid] = OneMap[unodeid] + 1;
...
int main(int argc, char* argv[]){
  ...
  if(ifs) {
      ...
      while(getline(ifs,buf)) {
        ...
        vecNode = graphrev[nodeid2];
        vecNode.push_back(nodeid1);
        graphrev[nodeid2] = vecNode;
      }
  } 
  else {
  ...
  for(i = 0; i<k ;i++) {
      ...
      compute_BFS_for_District(graphrev, OneMapSDrev, i, center); 
      string filename;
      ofstream ofs;
      stringstream ss;
      ss << i;
      filename = graphdata + string("~") + ss.str() + "dlevel";
      ...
      for(map<int,int>::iterator it=OneMapSD.begin(); it != OneMapSD.end(); it++) 
    {
      ...
      if(!OneMapSDrev[a])
         ...
         dis = OneMapSDrev[a];
      //ofs << it->first << " " << dis << " " << it->second << endl;
      ofs << it->first << " " << dis << " "  << it->second << endl;
    }
      ofs.close();
